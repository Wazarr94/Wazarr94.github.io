...
fnHax.addPlayer(game, "BOT1", "1",
                utilsHax.haxballVal['Team']["RED"], None, fnHax.resolveBotInputs1)
fnHax.addPlayer(game, "BOT2", "1",
                utilsHax.haxballVal['Team']["BLUE"], None, fnHax.resolveBotInputs2)

inputArrayCurr = [[[p.name, p.avatar, p.team["id"]], []] for p in game.players]

fnHax.resetPositionDiscs(game)

arrayRec = None
while arrayRec == None:
    arrayRec = fnHax.draw(currentFrame, inputArrayCurr, game, scorableDiscsId, scorableDiscsPos)
